...
export default function App() {
  ...
  }, [token, user, keys, authHeaders, API_URL, chatMode]);
  ...
  }, [token, fetchChats, notificationsEnabled, WS_URL]);
  ...
  }, [authStep, qrSession, tAuth, ta, API_URL]);
  ...
  const sendMessage = async (
    ...
    if (peer && chatMode === "cloud") {
      if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
        setStatus("No connection."); return;
      }
  ...
  const handleFile = async (file: File | null) => {
    if (file && peer && chatMode === "cloud") {
      try {
        ...
        if (!res.ok) { setStatus("File upload failed."); return; }
        const data = await res.json();
        await sendMessage("file", "", {
          fileName: file.name, fileType: file.type,
          fileId: data.fileId
        });
      } catch { setStatus("File upload failed."); }
      return;
    }
  ...
  const stopScreenShare = async () => {
    ...
    try {
      ...
      setStatus("РќРµ РІРґР°Р»РѕСЃСЏ Р·Р°РїСѓСЃС‚РёС‚Рё РґР·РІС–РЅРѕРє. РџРµСЂРµРІС–СЂС‚Рµ РґРѕСЃС‚СѓРї РґРѕ РјС–РєСЂРѕС„РѕРЅР°.");
    }
  ...
  const ensureCallWindow = (isVideo: boolean) => {
    ...
    const peerDisplay = peer && peer.id === currentPeerId ? peer.login ?? peer.phone : "Абонент";
  ...
  const acceptCall = async () => {
    ...
    if (!callerId) { setStatus("Missing caller data."); return; }
            ...
            <>
              ...
              {visibleMessages.some((m) => m.pinned) && (
                <div className="pinned-bar" onClick={() => {
                  ...
                  📌 {visibleMessages.filter((m) => m.pinned).length} закріплене повідомлення
                </div>
              )}
